...
- Output Item/헤더/본문 렌더: `src/view/SummarOutputManager.ts`, `src/view/OutputHeaderComposer.ts`
...
- div.summarview-output                                                   ← 스크롤 컨테이너
  - div.output-item[output-key="..."] (여러 개)
    ...
    - div.output-text
    - div.conversation-item[data-role="user|assistant"] (0개 이상, append 순서대로)
...
- div.summarview-composer
  - div.composer-header[data-label="compose prompt"|"reply to: ..."]
    ...
    - (좌) div.composer-model-chip[data-model="..."]
    - (우) div (spacer)
...
- div.sticky-header-container.sticky-header[data-key="..."]    ← 떠 있는 헤더 레이어(필요 시 생성/표시)
  - div.output-header[data-label="..."] (Sticky 복제 헤더)
    - (구성은 output-header와 동일; Reply 노출은 Composer 가용성에 따름)
```
...
## 상태/데이터 모델
- `SummarOutputRecord`: DOM 참조, 라벨, 노트명, 접힘 상태, 대화(conversations: conversation/output) 및 임시 결과(_tempResult) 관리
  - 최종 결과는 `OUTPUT` 타입 메시지로 보관, 중간 결과는 `_tempResult`로 분리
...
- Output Item/헤더/본문: `src/view/SummarOutputManager.ts`, `src/view/OutputHeaderComposer.ts`
- Sticky Header: `src/view/SummarStickyHeaderManager.ts`
- Composer: `src/view/SummarComposerManager.ts`
